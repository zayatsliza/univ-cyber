...
def run_dijkstra(
    ...
    labyrinth_shortest_paths: list[list[list[MazeLocation] | None]] = []
    ...
    while len(heap) > 0:
        ...
        for adj_location in min_dist_node.adj(
            ...
            if not labyrinth_visited[adj_location.i][adj_location.j]:
                ...
                path_copy.append(MazeLocation(min_dist_node.i, min_dist_node.j))
                heapq.heappush(
                    heap,
                    Node(
                        i=adj_location.i,
    ...
    labyrinth_shortest_paths[target.i][target.j].append(MazeLocation(target.i, target.j))
